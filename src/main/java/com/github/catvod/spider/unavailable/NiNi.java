...
public class NiNi extends Spider {
    ...
    public void init(String extend) {
        adult = !StringUtils.isEmpty(extend);
    }
    ...
    public String categoryContent(String tid, String pg, boolean filter, HashMap<String, String> extend) throws Exception {
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            String name = item.getString("showTitle");
            String pic = item.getString("showImg");
            if (land) list.add(new Vod(id, name, getPic(pic), getRemarks(item)));
    ...
    public String detailContent(List<String> ids) throws Exception {
        ...
        for (int j = 0; j < plays.length(); j++) {
            ...
            episodeName = StringUtils.isEmpty(episodeName) ? "播放" : episodeName;
        ...
        vod.setVodPlayUrl(StringUtils.join(vodItems, "#"));
    ...
    private String getRemarks(JSONObject item) {
        ...
        remark = StringUtils.isEmpty(remark) || remark.equals("null") ? "" : remark;
        return remark;
    }
    ...
    private DES getDes() {
        try {
            return SecureUtil.des(des_key.getBytes());
        } catch (Exception e) {
            SpiderDebug.log(e);
            return null;
        }
    ...
    private String encode(String data) {
        try {
            ...
            DES des = getDes();
            if (des == null) return "";
            return des.encryptHex(data.getBytes("UTF-8"));
        } catch (Exception e) {
            return "";
        }
    }
