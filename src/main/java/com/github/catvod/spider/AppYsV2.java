...
public class AppYsV2 extends Spider {
    ...
    public String homeContent(boolean filter) throws Exception {
        ...
        if (jsonArray != null) {
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                int typeId = jObj.getInt("type_id");
                Class newCls = new Class(typeId + "", typeName);
                ...
                if (filter) {
                    ...
                    List<Filter> filterArr = new ArrayList<>();
                    for (int k = url.isEmpty() ? 1 : 0; k < filters.length; k++) {
                        ...
                        List<Filter.Value> valueArr = new ArrayList<>();
                        for (int j = 1; j < oneLine.length; j++) {
                            Filter.Value kvo =null;
                            String kv = oneLine[j].trim();
                            int sp = kv.indexOf("=");
                            if (sp == -1) {
                                kvo=new Filter.Value(kv,kv);

                            } else {
                                String n = kv.substring(0, sp);
                                kvo=new Filter.Value( n.trim(), kv.substring(sp + 1).trim());

                            }
                            valueArr.add(kvo);
                        }
                        Filter jOne=new Filter(type,typeN,valueArr);

                        filterArr.add(jOne);
                    }

                    filterMap.put(typeId+"",filterArr);
                }
                classes.add(newCls);
            }
        }

        return Result.string(classes, homeVideos(),filterMap);
    }


    private List<Vod> homeVideos() {
        ...
        if (isTV) {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                Vod vod = new Vod();
                vod.setVodId(vObj.getString("nextlink"));
                vod.setVodName(vObj.getString("title"));
                vod.setVodPic(vObj.getString("pic"));
                vod.setVodRemarks(vObj.getString("state"));
                videos.add(vod);
            }
        } else {
            ...
            for (JSONArray jsonArray : arrays) {
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject vObj = jsonArray.getJSONObject(i);
                    int vid = vObj.getInt("vod_id");
                    if (ids.contains(vid)) continue;
                    ids.add(vid+"");
                    Vod vod = new Vod();
                    vod.setVodId(vObj.get("vod_id")+"");
                    vod.setVodName(vObj.getString("vod_name"));
                    vod.setVodPic(vObj.getString("vod_pic"));
                    vod.setVodRemarks(vObj.getString("vod_name"));
                    videos.add(vod);
                }
            }
        }
        ...
        if (jsonArray != null) {
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                if (vObj.has("vod_id")) {
                    ...
                    v.put("vod_id", vObj.getInt("vod_id")+"");
    ...
    public String searchContent(String key, boolean quick) throws Exception {
        String apiUrl = getApiUrl();
        String url = getSearchUrl(apiUrl, URLEncodeUtil.encode(key));
        String json = OkHttp.string(url, getHeaders(url));
        JSONObject obj = new JSONObject(json);
        JSONArray jsonArray = null;
        JSONArray videos = new JSONArray();
